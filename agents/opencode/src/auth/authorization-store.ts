...
/** A rotated credential that is durable here but not yet committed on the server. */
export interface PendingConnectorCredential {
  credential: string
  activateBy: string
}
...
export interface ConnectorAuthorization {
  ...
  pending?: PendingConnectorCredential
  linkedAt?: string
...
function parseAuthorization(value: unknown): ConnectorAuthorization {
  ...
  const allowed = new Set(["version", "serviceOrigin", "connectorId", "connectorKeyId", "credential", "credentialExpiresAt", "pending", "linkedAt", "trustedClient"]);
  ...
  if (
    // Version 1 predates credential rotation and upgrades in place: it is simply an
    // authorization with no pending credential, so an existing pairing survives the change.
...
function parsePending(value: unknown): PendingConnectorCredential {
  if (!value || typeof value !== "object") throw new Error("Pending connector credential has an invalid format");
  const record = value as Record<string, unknown>;
  const allowed = new Set(["credential", "activateBy"]);
  if (Object.keys(record).some((key) => !allowed.has(key))) {
    throw new Error("Pending connector credential contains unknown fields");
  }
  if (
    typeof record.credential !== "string" || !/^orc_[A-Za-z0-9_-]{43}$/u.test(record.credential) ||
    typeof record.activateBy !== "string" || Number.isNaN(Date.parse(record.activateBy))
  ) {
    throw new Error("Pending connector credential has an invalid format");
  }
  return { credential: record.credential, activateBy: record.activateBy };
}
