...
export class RemoteAPIClient {
  ...
  /** Issues a replacement credential that authorizes nothing until it is activated. */
  async rotateConnectorCredential(
    connectorCredential: string,
    signal?: AbortSignal,
  ): Promise<{ credential: string; activateBy: string }> {
    const record = objectRecord(await this.#request("/v1/connectors/self/rotate", {
      method: "POST",
      headers: { Authorization: `Bearer ${connectorCredential}` },
      ...(signal ? { signal } : {}),
    }));
    return {
      credential: requiredString(record, "credential"),
      activateBy: requiredDate(record, "activateBy"),
    };
  }

  /** Commits a rotation, authenticated by the pending credential rather than the live one. */
  async activateConnectorCredential(
    pendingCredential: string,
    signal?: AbortSignal,
  ): Promise<{ credentialExpiresAt: string }> {
    const record = objectRecord(await this.#request("/v1/connectors/self/rotate/activate", {
      method: "POST",
      headers: { Authorization: `Bearer ${pendingCredential}` },
      ...(signal ? { signal } : {}),
    }));
    return { credentialExpiresAt: requiredDate(record, "credentialExpiresAt") };
  }
