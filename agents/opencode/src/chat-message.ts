...
import type { ChatImage, ChatMessagePart, ChatPermission, ChatSubtask, ChatTodo, ChatTool } from "@openremotecode/protocol"
...
const toolOperations: Record<string, ChatTool["operation"]> = { read: "read", edit: "edit", write: "write",
  grep: "search", glob: "search", list: "list", bash: "execute", webfetch: "fetch" }
...
function toolSummary(part: ToolPart, directory?: string): ChatTool {
...
export function chatMessageContent(role: string, nativeParts: Part[], subtasks?: ReadonlyMap<string, ChatSubtask>,
  options: { includeTools?: boolean; includeShell?: boolean; includeActivities?: boolean; messageFinished?: boolean;
    directory?: string; images?: ReadonlyMap<string, ChatImage> } = {}) {
  const parts: ChatMessagePart[] = []
  let remaining = 48000
  let truncated = false
  for (const part of nativeParts) {
    ...
    if (part.type === "reasoning") {
      ...
      const tool = toolSummary(part, options.directory)
  ...
  if (options.includeActivities) {
    for (const part of parts) {
      ...
      const activity = native && activityFor(native, options.messageFinished)
