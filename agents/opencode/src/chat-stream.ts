...
export class ChatStreams {
  ...
  async subscribe(target: ChatStreamTarget): Promise<ChatStreamUpdate> {
    ...
    if (!item) {
      if (this.#items.size >= 2 || (readers.get(this.reader) ?? 0) >= 2 ||
          (watchers.get(this.reader) ?? 0) >= 2) throw new ChatAccessError("context_expired")
  ...
  async #read(item: Subscription, reconcile = false): Promise<Snapshot> {
    ...
    if (count >= 2) throw new ChatAccessError("context_expired")
